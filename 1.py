...
class PartTimeAPI:
    ...
    def send_sms_code(self, phone, captcha_code, area_code="+1"):
        ...
        try:
            ...
            sms_code = j.get("data", {}).get("smsCode")
            if sms_code:
                log_message("INFO", f"{self.log_prefix}SMS-код получен автоматически", "api")
...
def handle_quick_registration_captcha(message):
    ...
    if stage == "captcha":
        ...
        if ok:
            ...
            if sms_code:
                ok_token, sms_token = api.verify_sms_code(phone, sms_code, "+1")
                if ok_token:
                    ok, reg_resp = api.register_account(phone, sms_token, phone, "+1")
                    if ok:
                        settings = load_json(SETTINGS_FILE)
                        saved = save_account(
                            phone,
                        ...
                        if saved:
                            token = reg_resp.get("data", {}).get("token", "")
                            if token:
                                update_account_token(phone, token)
                            bot.send_message(
                                ADMIN_ID,
                                f"✅ Зарегистрирован: +1{phone}  (пароль = номер).",
                            )
                        else:
                            bot.send_message(ADMIN_ID, "❌ Аккаунт уже существует.")
                        quick_registration_states.pop(message.from_user.id, None)
                        return
                    bot.send_message(ADMIN_ID, f"❌ Регистрация не удалась: {reg_resp}")
                    quick_registration_states.pop(message.from_user.id, None)
                    return
                bot.send_message(ADMIN_ID, f"❌ Ошибка проверки SMS: {sms_token}")
                st["stage"] = "sms"
                bot.send_message(ADMIN_ID, "Введите код из SMS вручную:")
                return
            st["stage"] = "sms"
    ...
    if stage == "captcha":
        ...
        if ok:
            ...
            if sms_code:
                ok_token, sms_token = api.verify_sms_code(phone, sms_code, "+1")
                if ok_token:
                    ok, reg_resp = api.register_account(phone, sms_token, password, "+1")
